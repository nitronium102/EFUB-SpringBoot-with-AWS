...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException
	{
		...
		//현재 로그인 중인 서비스를 구분하는 코드 , 구글만 사용함
		...
		httpSession.setAttribute("user", new SessionUser(user));//세션 정보를 저장하기 위한 dto
	...
	private User saveOrUpdate(OAuthAttributes attributes) {//구글 사용자 정보가 업데이트 되었을 때 변경 사항이 User 엔티티에도 반영
